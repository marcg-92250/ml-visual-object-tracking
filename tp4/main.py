"""
File name: main.py
Description: Main script for Appearance-Aware IoU-Kalman Tracking with ReID
Python Version: 3.7+
"""
...
def main():
    """
    Main function for ReID-Kalman-IoU tracking.
    """
    ...
    # alpha: weight for IoU, beta: weight for ReID similarity
    tracker = IOUReIDKalmanTracker(
        iou_threshold=0.3,
        max_age=30,
        alpha=0.6,  # IoU weight
        beta=0.4    # ReID similarity weight
    )
    ...
    print("Starting ReID-Kalman-IoU tracking...")
    ...
    for frame_idx, img_file in enumerate(img_files, start=1):
        ...
        # Display frame
        cv2.imshow('ReID-Kalman-IoU Tracking', frame)
